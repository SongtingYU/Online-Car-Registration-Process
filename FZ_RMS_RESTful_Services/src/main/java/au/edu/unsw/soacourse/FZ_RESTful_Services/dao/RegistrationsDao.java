...
public enum RegistrationsDao {
	...
	public List<EntryBean> getRegistrations() {
		List<EntryBean> results = new ArrayList<EntryBean>();
		NodeList nodeList = null;
		try {
			nodeList = (NodeList) xpath.evaluate("Registrations/child::Entry",
	...
	public boolean insertEntry(EntryBean entry) {
		if (queryEntry(entry.get_rid()) != null)
			return false;
		Element root = doc.getDocumentElement();
		Element u = doc.createElement("Entry");
		root.appendChild(u);
		for (Map.Entry<String, String> a : entry.getMap().entrySet()) {
			...
			if (!"$".equals(value)) {
				...
				Element k = doc.createElement(key);
				u.appendChild(k);
				for (Map.Entry<String, String> b : entry.getDriver().getMap()
						.entrySet()) {
	...
	public boolean updateEntry(EntryBean entry) {
		NodeList nodeList = null;
		try {
			nodeList = (NodeList) xpath.evaluate(
	...
	public boolean deleteEntry(String _rid) {
		NodeList nodeList = null;
		try {
			nodeList = (NodeList) xpath.evaluate(
					"Registrations/child::Entry[_rid='" + _rid + "']", doc,
